...
# Plot Lyapunov derivative
V_dot = np.gradient(V, sol.t)
line3, = axs[2].plot(sol.t, V_dot, label='Lyapunov function derivative $\dot{V}$')
axs[2].axhline(0, color='k', linestyle='--')
axs[2].set_xlabel('Time (s)')
axs[2].set_ylabel('$\\dot{V}$')
...
# Initialize marker handles as None
stab_marker = None
theta_marker = None
theta_dot_marker = None
...
def update(val):
    global stab_marker, theta_marker, theta_dot_marker
    v0 = slider_v0.val
    ...
    if stab_marker is not None:
        stab_marker.remove()
        stab_marker = None
    if theta_marker is not None:
        theta_marker.remove()
        theta_marker = None
    if theta_dot_marker is not None:
        theta_dot_marker.remove()
        theta_dot_marker = None
    # Add new markers if stabilized
    if t_stab is not None:
        stab_marker = axs[0].scatter(t_stab, lyapunov(theta_stab, theta_dot_stab), color='red', label='Stabilization point')
        theta_marker = axs[1].scatter(t_stab, theta_stab, color='red', marker='o', label='Theta at stabilization')
        theta_dot_marker = axs[1].scatter(t_stab, theta_dot_stab, color='green', marker='o', label='Theta dot at stabilization')
    axs[0].relim()
